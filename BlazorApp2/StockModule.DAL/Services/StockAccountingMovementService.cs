...
namespace StockModule.DAL.Services
{
    public class StockAccountingMovementService : EntityService<StockAccountingMovement>, IStockAccountingMovementService
    {
        ...
        public StockAccountingMovement? GetFirstByReasonAndStatus(int reasonId, int[] inStatus, int? fromStockId, int? toStockId, double quantity)
        {
           return Set.Include(_ => _.StockMovement).Where(_ => inStatus.Any(__=>__==_.Status) && _.StockMovement.StockMovementReason.Id == reasonId).OrderBy(_ => _.Status).FirstOrDefault(_ => _.StockMovement.FromStockId == fromStockId && _.StockMovement.ToStockId == toStockId && _.StockMovement.Quantity == quantity);
        }
